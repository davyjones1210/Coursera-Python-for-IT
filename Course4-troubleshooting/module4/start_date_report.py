...
def get_same_or_newer(start_date):
    ...
    # Preprocess and cache the data on the first call
    if not hasattr(get_same_or_newer, "all_rows"):
        data = get_file_lines(FILE_URL)
        get_same_or_newer.all_rows = list(csv.reader(data[1:]))

    reader = get_same_or_newer.all_rows

    # We want all employees that started at the same date or the closest n$
    # date. To calculate that, we go through all the data and find the
    # employees that started on the smallest date that's equal or bigger t$
